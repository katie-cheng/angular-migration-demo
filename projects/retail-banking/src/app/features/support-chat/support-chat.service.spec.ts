...
describe('SupportChatService', () => {
  ...
  it('requests the first page by default', () => {
    ...
    const request = http.expectOne((req) => req.url === '/api/support-chat');
  ...
  it('caches the first page', () => {
    ...
    http.expectOne((req) => req.url === '/api/support-chat').flush({
      items: [],
      total: 0,
      page: 1,
      pageSize: 25,
  ...
  it('posts new records and puts existing ones', () => {
    ...
    expect(http.expectOne('/api/support-chat').request.method).toBe('POST');
    http.verify();

    service.save({ id: 'x-1' } as never).subscribe();
    expect(http.expectOne('/api/support-chat/x-1').request.method).toBe('PUT');
  });
});
