...
describe('MortgageListComponent', () => {
  ...
  let service: jasmine.SpyObj<MortgageService>;

  beforeEach(async () => {
    service = jasmine.createSpyObj<MortgageService>('MortgageService', ['list', 'get', 'save', 'remove', 'first', 'count']);
    service.list.and.returnValue(of({ items: [], total: 0, page: 1, pageSize: 25 }));

    await TestBed.configureTestingModule({
      declarations: [MortgageListComponent],
      imports: [HttpClientTestingModule, ReactiveFormsModule, RouterTestingModule],
      providers: [{ provide: MortgageService, useValue: service }],
